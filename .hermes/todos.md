...
- [x] sales-stepper. `Stepper` ja integrado no funil de vendas em /sales (apps/web/src/app/[locale]/sales/page.tsx:241, secao "Funil de vendas"). Componente criado no swarm e ja usado na feature de vendas. (concluido)
- [x] deploy-design. `vercel deploy --prod` (commit 975170d) -> landmap-iljzm23tj (READY, build remoto Linux iad1). APIs 200 (health/stats/cities/POST search); paginas 404 = issue pre-existente de routing/locale (ja anotado em paginas-404-vercel, NAO do refactor). (concluido)
- [x] pages-404-fix. RESOLVIDO (commit 71da3f0 -> deploy landmap-21bnp36iw Ready). Causa: middleware.ts estava em apps/web/middleware.ts (FORA de src/), entao o Next nao o detectava -> locale nunca resolvido -> root layout getLocale() lancava 'Unable to find a locale' -> 404 em todas as paginas. Fix: moveu middleware para apps/web/src/middleware.ts + unstable_setRequestLocale no [locale]/layout.tsx. Validado: / -> 307 (redirect), /pt-BR /en-US /es-ES /pt-BR/search -> 200, /api/health -> 200. (concluido)

- [x] memory-file. Criar MEMORY.md (memoria mestra do projeto) + atualizar CLAUDE.md com ponteiro/status. Captura stack, mono-repo, rotas, WIP Mundo 3D, debitos (DNS/Tailwind v4/tokens) e convencoes. (concluido)
...
- [ ] dominios-ns-automacao. Tentativa de automacao do NS (2026-07-10): sem credenciais de registrador no ambiente (so Vercel). NS em registrador exige auth do registrador - barra de seguranca, sem API gratuita de terceiro. Caminho: (a) manual ~1min no painel de cada registrador trocando NS para ns1/ns2.vercel-dns.com; ou (b) API do Registro.br com certificado de acesso gratuito. Pende token do usuario. (aguarda credencial)

- [x] deploy-final. vercel deploy --prod (commits 62be925 [/live 404 fix] + 27a2dc5 + 4d3de56 [script NS Registro.br]) -> landmap-eso2p38hx (READY, build remoto Linux ~1m). Inclui o fix do /live (404->200) que ainda nao estava em producao. Validacao: /api/health|stats|cities 200; /pt-BR /pt-BR/search /pt-BR/live /pt-BR/world /en-US /es-ES 200. Alias prod landmap-dizzys-projects-d5a44b36.vercel.app tambem servindo o novo deploy. (concluido)
- [ ] dominios-ns-registrador / dominios-ns-automacao. Unico pendente = ACAO EXTERNA no registrador (trocar NS para ns1/ns2.vercel-dns.com). Sem credencial de registrador no ambiente, fica fora do alcance do agente. (aguarda acao/credencial do usuario)

- [x] ux-yc-elevate. Continuacao da elevacao UX/UI para padrao YC/Valley (commit 7458948): (1) Navbar agrupa 11 links em menu \"Mercado\" (dropdown acessivel: aria-haspopup/expanded, Escape/click-fora, foco retornado ao botao; mobile mantem lista); (2) `template.tsx` com transicao de pagina sutil (fade+rise, reduced-motion aware via `.page-enter` em globals.css); (3) `viewport` export com themeColor #050505; (4) teste de build do docs ganha timeout 120s (suíte 100% verde). typecheck + lint do web VERDES. PyTorch: suíte Python 11/11 verde (refinador treinado, artifacts atuais). (concluido)
- [x] deploy-yc. `vercel deploy --prod` apos melhorias UX/UI -> landmapon-5c8s6e72p (READY, build remoto Linux ~2m). Alias landmap.us.kg (SSL async p/ landmap.com.br/getlandmap.app/us.kg). Validacao live: /pt-BR /en-US /es-ES /search /world -> 200; /api/health|stats|cities -> 200; menu "Mercado" presente no HTML. Suíte 308/308 verde. (concluido)
