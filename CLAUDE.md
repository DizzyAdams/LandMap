...
- DNS: lado Vercel 100% (apex+www de `landmap.com.br`/`landmap.us.kg`/`getlandmap.app` atribuídos ao projeto + registros A 76.76.21.21 / www CNAME cname.vercel-dns.com criados via `vercel dns add`). **Falta apenas trocar o NS no registrador** para `ns1/ns2.vercel-dns.com` (ação externa). Produção live em `landmap-dizzys-projects-d5a44b36.vercel.app`.
