...
- **"Mundo 3D" (bmap.io-style):** ⚠️ **NÃO está no app ativo.** Os arquivos (`lib/bmap.ts`, `components/BmapViewer|SkylineCanvas|EnergyPanel|LivePulse|InvestmentCard`, `app/[locale]/world/page.tsx`) foram arquivados em `archive_landmap_only_2026-07-14/` e NÃO fazem parte do build/deploy atual. O app ativo usa o mapa Híbrido C (`/map`) + `/dashboard` (ver DESIGN.md §4.1). A paleta emerald/cyan/gold citada só sobrevive hoje em `packages/gamification` + `packages/ui` (MetricStat/CommandPalette) como *feature palette* documentada, não no mapa.
