...
export default async function EmbeddingDocsPage({ params }: { params: Promise<{ locale?: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#050505]">
      <div className="mx-auto max-w-4xl px-6 py-20">
        {/* Breadcrumb */}
        <nav className="mb-8 flex items-center gap-2 text-xs text-[var(--muted-foreground-lovable)]">
          <a href={`/${locale}/docs`} className="transition hover:text-[var(--muted-foreground-lovable)]">Docs</a>
          <span>/</span>
          <span className="text-[var(--muted-foreground-lovable)]">Embeddings</span>
        </nav>
        ...
        <section className="mt-10">
          <h2 className="text-lg font-medium text-[var(--foreground)]">Como funciona</h2>
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            <div className="rounded-xl border border-[var(--border-lovable)] bg-[var(--card)]/40 p-5">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--primary)]/10 text-[var(--primary)] text-sm font-bold">1</div>
              <h3 className="mt-3 text-sm font-medium text-[var(--foreground)]">Tokenização</h3>
        ...
        <section className="mt-12">
          <h2 className="text-lg font-medium text-[var(--foreground)]">Endpoint</h2>
          <div className="rounded-2xl border border-[var(--border-lovable)] bg-[var(--card)] p-4 mt-4 p-5">
            <div className="flex items-center gap-3">
              <span className="shrink-0 rounded-md bg-[var(--primary)]/10 px-2 py-0.5 font-mono text-[11px] font-semibold text-[var(--primary)]">GET</span>
              <code className="text-xs text-[var(--muted-foreground-lovable)] font-mono">/api/embeddings/similar</code>
            </div>
            <div className="mt-4">
              <h3 className="text-sm font-medium text-[var(--muted-foreground-lovable)]">Parâmetros</h3>
              <table className="mt-2 w-full text-xs">
                <thead>
                  <tr className="border-b border-[var(--border-lovable)] text-[var(--muted-foreground-lovable)]">
                ...
                <tbody className="text-[var(--muted-foreground-lovable)]">
                  <tr className="border-b border-[var(--border-lovable)]/50">
                    <td className="py-2 font-mono">q</td>
                    <td className="py-2 text-[var(--muted-foreground-lovable)]">string</td>
                    <td className="py-2 text-[var(--muted-foreground-lovable)]">Sim</td>
                    <td className="py-2">Texto da consulta em linguagem natural</td>
                  </tr>
                  <tr className="border-b border-[var(--border-lovable)]/50">
                    <td className="py-2 font-mono">limit</td>
                    <td className="py-2 text-[var(--muted-foreground-lovable)]">number</td>
                    <td className="py-2 text-[var(--muted-foreground-lovable)]">Não</td>
            ...
            <div className="mt-4">
              <h3 className="text-sm font-medium text-[var(--muted-foreground-lovable)]">Exemplo</h3>
              <pre className="mt-2 overflow-x-auto rounded-md bg-[var(--card)] p-3 text-xs text-[var(--muted-foreground-lovable)]">
                <code>{`curl "http://localhost:4000/embeddings/similar?q=apartamento+centro+curitiba&limit=5"`}</code>
              </pre>
            </div>
            <div className="mt-4">
              <h3 className="text-sm font-medium text-[var(--muted-foreground-lovable)]">Resposta</h3>
              <pre className="mt-2 overflow-x-auto rounded-md bg-[var(--card)] p-3 text-xs text-[var(--muted-foreground-lovable)]">
                <code>{`{
  "items": [
    {
  ...
  "total": 3
        ...
        {/* Use cases */}
        <section className="mt-12">
          <h2 className="text-lg font-medium text-[var(--foreground)]">Casos de uso</h2>
          <div className="mt-4 space-y-3">
            {[
              ...
              <div key={item.title} className="rounded-xl border border-[var(--border-lovable)] bg-[var(--card)]/40 p-4">
                <h3 className="text-sm font-medium text-[var(--foreground)]">{item.title}</h3>
                <p className="mt-1 text-xs text-[var(--muted-foreground-lovable)]">{item.desc}</p>
              </div>
            ))}
          </div>
        </section>
