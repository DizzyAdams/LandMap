  ...
  return (
    <main className="min-h-screen grid-bg px-6 py-16">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-2 h-4 w-32 animate-pulse rounded bg-neutral-800/60" />
        <ul role="list" className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <li key={i} className="h-32 animate-pulse rounded-xl border border-neutral-800 bg-neutral-900/40" />
          ))}
        </ul>
      </div>
    </main>
