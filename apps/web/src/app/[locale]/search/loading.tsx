  ...
  return (
    <main className="min-h-screen grid-bg text-[var(--foreground)]">
      <section className="mx-auto max-w-6xl px-6 py-16">
        ...
        <div className="mt-8 rounded-xl border border-[var(--border-lovable)] bg-[var(--card)]/40 p-4">
      ...
      <section className="mx-auto max-w-6xl px-6 pb-24">
        ...
        <ul role="list" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <li key={i} className="h-28 animate-pulse rounded-xl border border-[var(--border-lovable)] bg-[var(--card)]/40" />
          ))}
        </ul>
      </section>
    </main>
