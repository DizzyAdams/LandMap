      ...
      <div className="mb-6">
        ...
        <ul className="flex flex-col gap-2">
          {data.energy.topSolar.slice(0, 6).map((roof) => (
            <li key={roof.id} className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div className="flex items-center justify-between gap-2 mb-2">
                <span className="truncate font-medium text-neutral-100">{roof.id}</span>
              ...
              <div className="grid grid-cols-3 gap-x-3 gap-y-1 text-xs">
                <div className="flex flex-col">
                  <span className="text-neutral-400">{t('energy.roof')}</span>
                  <span className="text-neutral-200 tabular-nums">{roof.roofM2} m²</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-neutral-400">{t('energy.kwp')}</span>
                  <span className="text-emerald-300 tabular-nums">{roof.kwProxy} kWp</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-neutral-400">{t('energy.co2')}</span>
                  <span className="text-neutral-200 tabular-nums">{roof.co2TonsYr} tCO₂/ano</span>
                </div>
              </div>
            </li>
          ))}
      ...
      <div className="mb-4">
        ...
        <ul className="flex flex-col gap-2">
          {data.thermal.hottest.slice(0, 4).map((cell) => (
            <li key={cell.id} className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div className="flex items-center justify-between gap-2 mb-2">
                <span className="truncate font-medium text-neutral-100">{cell.id}</span>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                <div className="flex justify-between gap-2">
                  <span className="text-neutral-400">{t('thermal.heat')}</span>
                  <span className="text-neutral-200 tabular-nums">{cell.heatIndex}</span>
                </div>
                <div className="flex justify-between gap-2">
                  <span className="text-neutral-400">{t('thermal.buildings')}</span>
                  <span className="text-neutral-200 tabular-nums">{cell.buildings}</span>
                </div>
              </div>
            </li>
          ))}
