...
export default function EnergyPanel({ data }: { data: WorldAnalysis }) {
  ...
  return (
    <section className="rounded-xl border border-[var(--border-lovable)] bg-[var(--card)] backdrop-blur-md p-4 text-sm text-[var(--foreground)]">
      ...
      <div className="mb-6">
        <div className="text-xs uppercase tracking-wider text-[var(--muted-foreground-lovable)] mb-2">
          {t('energy.topSolar')}
        </div>
        <ul className="flex flex-col gap-2">
          {data.energy.topSolar.slice(0, 6).map((roof) => (
            <li key={roof.id} className="rounded-lg border border-[var(--border-lovable)] bg-[var(--card)] p-3">
              ...
              <div className="grid grid-cols-3 gap-x-3 gap-y-1 text-xs">
                <div className="flex flex-col">
                  <span className="text-[var(--muted-foreground-lovable)]">{t('energy.roof')}</span>
                  <span className="text-[var(--foreground)] tabular-nums">{roof.roofM2} m²</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[var(--muted-foreground-lovable)]">{t('energy.kwp')}</span>
                  <span className="text-[var(--primary)] tabular-nums">{roof.kwProxy} kWp</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[var(--muted-foreground-lovable)]">{t('energy.co2')}</span>
      ...
      <div className="mb-4">
        ...
        <ul className="flex flex-col gap-2">
          {data.thermal.hottest.slice(0, 4).map((cell) => (
            <li key={cell.id} className="rounded-lg border border-[var(--border-lovable)] bg-[var(--card)] p-3">
              <div className="flex items-center justify-between gap-2 mb-2">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                <div className="flex justify-between gap-2">
                  <span className="text-[var(--muted-foreground-lovable)]">{t('thermal.heat')}</span>
                  <span className="text-[var(--foreground)] tabular-nums">{cell.heatIndex}</span>
                </div>
                <div className="flex justify-between gap-2">
                  <span className="text-[var(--muted-foreground-lovable)]">{t('thermal.buildings')}</span>
      ...
      <div>
        ...
        />
        <div className="flex justify-between text-[10px] text-[var(--muted-foreground-lovable)] mt-1">
