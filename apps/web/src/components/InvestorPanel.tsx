        ...
        {data.topByScore.slice(0, 6).map((opp) => (
          <li key={opp.id} className="rounded-lg border border-[var(--border-lovable)] bg-[var(--card)] p-3">
            <div className="flex items-center justify-between gap-2 mb-2">
              ...
              {opp.fundReady && (
                <span className="shrink-0 rounded-full bg-[var(--primary)]/15 text-[var(--primary)] text-[10px] font-medium px-2 py-0.5">
                  {t('investor.badge')}
                </span>
              )}
            </div>

            <div className="h-1.5 rounded-full bg-[var(--card)] overflow-hidden mb-3">
              <div
