  ...
  return (
    <div className="fixed inset-0 z-[120]" role="dialog" aria-modal="true" aria-label={label} onKeyDown={onKeyDown}>
      ...
      {rect && (
        <div
          aria-hidden
          className="pointer-events-none absolute rounded-lg ring-2 ring-[var(--primary)] transition-all duration-300 motion-reduce:transition-none"
          style={{
            top: rect.top - 6,
            left: rect.left - 6,
            width: rect.width + 12,
      ...
      <div
        ...
        className={cn(
          'surface absolute rounded-lg border border-[var(--border)] p-5 outline-none',
      ...
      >
        ...
        <h2 className="mt-1.5 text-base font-semibold text-[var(--foreground)]">{step.title}</h2>
        <p id={descId} className="mt-1.5 text-sm leading-relaxed text-[var(--muted-foreground)]">
          {step.description}
        </p>
        ...
        <div className="mt-5 flex items-center justify-between gap-2">
          <button
            type="button"
            onClick={skip}
            className="rounded-lg px-3 py-1.5 text-xs text-[var(--muted-foreground)] outline-none transition hover:text-[var(--muted-foreground)] focus-visible:shadow-[var(--ring)]"
          >
            Pular
          </button>
          <div className="flex items-center gap-2">
            {current > 0 && (
              <button
                type="button"
                onClick={prev}
                className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--muted-foreground)] outline-none transition hover:border-[var(--border)] hover:bg-[var(--accent)] focus-visible:shadow-[var(--ring)]"
              >
                Voltar
              </button>
            )}
            <button
              type="button"
              onClick={next}
              className="rounded-lg bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/70 px-4 py-1.5 text-xs font-semibold text-[var(--background)] outline-none transition hover:-translate-y-px focus-visible:shadow-[var(--ring)]"
            >
